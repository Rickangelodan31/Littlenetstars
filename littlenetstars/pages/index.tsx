...
export default function Home({ settings }: Props) {
  ...
  const ctaDesc =
    s.cta_text ||
    "Your child's first session is on us — no card needed. After that, it's just £40/month for Friday sessions.";
  const duration = s.duration || "1 Hour";
  ...
  const programmesSubtitle =
    s.home_programmes_subtitle ||
    "Beyond weekend sessions — camps, schools & nurseries";

  // How It Works text
  const howItWorksTitle = s.home_howitworks_title || "How It Works";
  const howItWorksSubtitle =
    s.home_howitworks_subtitle || "Three easy steps to get your child on court";
  const steps = [
    {
      step: "01",
      icon: s.home_step1_icon || "📅",
      title: s.home_step1_title || "Choose a Date",
      desc:
        s.home_step1_desc ||
        "Pick from available Friday sessions in your city, starting 17 July 2026.",
    },
    {
      step: "02",
      icon: s.home_step2_icon || "👧",
      title: s.home_step2_title || "Add Your Child",
      desc:
        s.home_step2_desc ||
        "Enter your child's details. You can add multiple children at once.",
    },
    {
      step: "03",
      icon: s.home_step3_icon || "✅",
      title: s.home_step3_title || "Secure Your Session",
      desc:
        s.home_step3_desc ||
        "Pay securely via card, Apple Pay, or Google Pay and you're done.",
    },
  ];
  ...
  const programmeSlides = [
      ...
      details: [
        {
          icon: "📅",
          label: "Dates",
          value: s.home_camp_dates || "Thu 1st – Fri 2nd April 2027",
        },
        {
          icon: "⏰",
          label: "Time",
          value: s.home_camp_time || "9:00am – 4:00pm",
        },
        {
          icon: "💰",
          label: "Full Camp (2 days)",
          value: s.home_camp_full || "£95.00",
        },
        {
          icon: "🎟️",
          label: "Single Day",
          value: s.home_camp_single || "£60.00",
        },
      ],
    ...
    },
    {
      ...
      description:
        s.home_schools_description ||
        "Little Netstars delivers tailored netball sessions in your school or nursery. Our coaches adapt every session to suit your setting — engaging, motivating, and fun for all.",
      details: [
        {
          icon: "👶",
          label: "Age Group",
        ...
        },
        {
          ...
          value: s.home_schools_length || "1 hour",
        },
        {
          icon: "📋",
          label: "Session Plans",
          value: s.home_schools_plans || "Tailored to your needs",
        },
        {
          icon: "🏫",
          label: "Delivery",
          value: s.home_schools_delivery || "Schools & nurseries",
        },
      ],
                ...
                {programmeSlides.map((slide, i) => (
                  <motion.div
                    ...
                    animate={{
                      ...
                      x: i === activeSlide ? 0 : i < activeSlide ? -40 : 40,
                    }}
                    transition={{ duration: 0.5, ease: "easeInOut" }}
                    className={`bg-gradient-to-br ${slide.bg} p-10 sm:p-14 ${i === activeSlide ? "block" : "hidden"}`}
                  >
                    <span
                      className={`inline-block text-xs font-bold px-3 py-1 rounded-full mb-4 uppercase tracking-widest ${slide.badgeBg}`}
                    >
                      {slide.tag}
                    </span>
                    ...
                    <div className="mt-8">
                      <Link
                        href={slide.cta.href}
                        className="inline-block bg-white hover:bg-slate-50 text-slate-900 font-bold px-8 py-3 rounded-full shadow-lg transition-transform hover:scale-105 text-sm"
                      >
                        {slide.cta.label} →
                      </Link>
                    </div>
              ...
              <div className="flex items-center justify-center gap-4 mt-6">
                <button
                  onClick={() =>
                    setActiveSlide(
                      (i) =>
                        (i - 1 + programmeSlides.length) %
                        programmeSlides.length,
                    )
                  }
                  className="p-2 rounded-full bg-slate-200 dark:bg-slate-700 hover:bg-purple-100 dark:hover:bg-slate-600 transition-colors"
                  aria-label="Previous"
                >
                  <svg
              ...
              >
                <div className="border-2 border-purple-200 dark:border-purple-800 rounded-2xl p-6 text-center">
                  <div className="text-2xl font-extrabold text-slate-900 dark:text-white">
                    £{(satPricePence / 100).toFixed(0)}
                  </div>
                  <div className="text-purple-600 dark:text-purple-400 font-semibold text-sm mt-1">
                    per month
                  </div>
                  <div className="mt-3 text-slate-600 dark:text-slate-300 text-sm font-medium">
                    All Fridays
                  </div>
                ...
                <div className="border-2 border-yellow-400 rounded-2xl p-6 text-center relative">
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-yellow-400 text-slate-900 text-xs font-bold px-3 py-0.5 rounded-full">
                    Best Value
                  </div>
                  <div className="text-2xl font-extrabold text-slate-900 dark:text-white">
                    £{(bothPricePence / 100).toFixed(0)}
                  </div>
                  <div className="text-yellow-600 dark:text-yellow-400 font-semibold text-sm mt-1">
                    per month
                  </div>
                  <div className="mt-3 text-slate-600 dark:text-slate-300 text-sm font-medium">
                    Coming Soon
